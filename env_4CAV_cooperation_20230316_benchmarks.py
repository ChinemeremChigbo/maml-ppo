...
class Network():
    def __init__(self):
        ...
        # self.trans_matrix = np.array([[0.8, 0.2, 0, 0, 0],
        #                             [0.1, 0.8, 0.1, 0, 0],
        #                             [0, 0.1, 0.8, 0.1, 0],
        #                             [0, 0, 0.1, 0.8, 0.1],
        #                             [0, 0, 0, 0.2, 0.8]])
    ...
    def reset(self):
        ...
        if band_basis == 7:
            self.Bandwidth_available = np.random.choice([6,7],1, [0.1, 0.9])
        elif band_basis == 2:
            self.Bandwidth_available = np.random.choice([2,3],1, [0.9, 0.1])
        else:
            self.Bandwidth_available = np.random.choice([band_basis-1, band_basis, band_basis+1], 1, [0.05, 0.9, 0.05])
        ...
        self.O_vehs_all = np.array([7,4,6,8])
        O_norm = (self.O_vehs_all - np.array([4,4,4,4]))/4
           
        
        ############################# Distance and channel gain ###############################################
        
        self.Distance_all = np.array([6,6,18,30])
        Distance_norm = (self.Distance_all - np.array([6,6,6,6]))/24
    ...
    def step(self, curr_action):
        ...
        ########################################################################### Brute force optimal ######################################################################################################
        # of all candidate action from 0000 to 1111
        switch_costs_BF_optimal = np.zeros([2,2,2,2])
        objs_BF_optimal = np.zeros([2,2,2,2])
        rewards_BF_optimal = np.zeros([2,2,2,2])
        for action_agent_0 in range(0,2):
            for action_agent_1 in range(0,2):
                for action_agent_2 in range(0,2):
                    for action_agent_3 in range(0,2):
                        actions_temp = np.array([action_agent_0, action_agent_1, action_agent_2, action_agent_3])
                        ...
                        switch_costs_BF_optimal[action_agent_0, action_agent_1, action_agent_2, action_agent_3] = switch_cost
                        objs_BF_optimal[action_agent_0, action_agent_1, action_agent_2, action_agent_3] = obj_opt
                        rewards_BF_optimal[action_agent_0, action_agent_1, action_agent_2, action_agent_3] = reward
        ...
        actions_BF_optimal = np.array([actions_BF_optimal_cand[0][0], actions_BF_optimal_cand[1][0], actions_BF_optimal_cand[2][0], actions_BF_optimal_cand[3][0]])
        ...
        actions_random = np.random.randint(2, size=4)
        ...
        actions = np.array([np.argmax(actions_array[0]), np.argmax(actions_array[1]), np.argmax(actions_array[2]), np.argmax(actions_array[3])])
        ...
        if obj_opt == -1:           
            obj_modified = 0
            actions_modified = np.array([0,0,0,0])           
