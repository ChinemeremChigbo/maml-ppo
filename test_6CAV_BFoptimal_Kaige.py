...
from tf2marl.common.logger_benchmark_2 import RLLogger
from tf2marl.agents import MADDPGAgent, MADDPGAgent_Kaige, MADDPGAgent_Kaige_6CAV, MATD3Agent, MASACAgent, MAD3PGAgent
...
from env_6CAV_cooperation_20230319_benchmarks import Network
...
def train_config():
    ...
    num_units_c = 64*3                # units per hidden layer
    num_units_a = 64
    num_units = 64
...
def train(_run, exp_name, save_rate, display, restore_fp,
    ...
    while True:
        ...
        # print("action_n:", action_n)
        new_obs_n, rew_n, done_n, obj_n, switch_n, reward_modified = env.step(action_n)
        ...
        if len(logger.episode_rewards) > num_episodes:
            ...
            logger.experiment_end(q_loss_all,pol_loss_all,train_steps_plot,episode_rewards_modified)
...
def get_agents(_run, env, num_adversaries, good_policy, adv_policy, lr, lr_c, lr_a, batch_size,
               buff_size, num_units, num_units_c, num_units_a, num_layers, gamma, tau, priori_replay, alpha, num_episodes,
    ...
    for agent_idx in range(num_adversaries):
        if adv_policy == 'maddpg':
            ...
            #                     _run=_run)
            agent = MADDPGAgent_Kaige_6CAV(env.observation_space, env.action_space, agent_idx, batch_size,
                                buff_size,
                                lr_c, lr_a, num_layers,
                                num_units_c, num_units_a, gamma, tau, priori_replay, alpha=alpha,
                                max_step=num_episodes * max_episode_len, initial_beta=beta,
                                _run=_run)
