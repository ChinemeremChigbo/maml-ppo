...
class Network():
    def __init__(self):
        ...
        self.obj_opt_6CAV = np.load('KKT_opt/KKT_6CAV_opt_data.npy') 
        print(self.obj_opt_6CAV.shape)
        self.obj_opt_5CAV = np.squeeze(sio.loadmat('KKT_opt/KKT_5CAV_opt_data.mat').get('KKT_obj_5CAV_all'))
        print(self.obj_opt_5CAV.shape)
        self.obj_opt_4CAV = np.squeeze(sio.loadmat('KKT_opt/KKT_4CAV_opt_data.mat').get('KKT_obj_4CAV'))
        print(self.obj_opt_4CAV.shape)
        self.obj_opt_3CAV = np.squeeze(sio.loadmat('KKT_opt/KKT_3CAV_opt_data_all.mat').get('KKT_obj_3CAV_all'))
        print(self.obj_opt_3CAV.shape)
    ...
    def reset(self):
        ...
        O_norm = (self.O_vehs_all - np.array([4,4,4,4,4,4]))/4
           
        
        ############################# Distance and channel gain ###############################################
        
        self.Distance_all = 6*np.ones(self.CAV_pair_num)
        Distance_norm = (self.Distance_all - np.array([6,6,6,6,6,6]))/24
    ...
    def step(self, curr_action):
        ...
        actions = np.array([np.argmax(actions_array[0]), np.argmax(actions_array[1]), np.argmax(actions_array[2]),np.argmax(actions_array[3]),np.argmax(actions_array[4]),np.argmax(actions_array[5])])
        ...
        if Activated_CAV_pair_num == 6:
            obj_opt = self.obj_opt_6CAV[int(self.Bandwidth_available-2), int(O_vehs_ordered[0]-4), int(O_vehs_ordered[1]-4), int(O_vehs_ordered[2]-4), int(O_vehs_ordered[3]-4), int(O_vehs_ordered[4]-4), int(O_vehs_ordered[5]-4), int(Distance_ordered[0]/6-1), int(Distance_ordered[1]/6-1), int(Distance_ordered[2]/6-1), int(Distance_ordered[3]/6-1), int(Distance_ordered[4]/6-1), int(Distance_ordered[5]/6-1)]
        elif Activated_CAV_pair_num == 5:
            obj_opt = self.obj_opt_5CAV[int(self.Bandwidth_available-2), int(O_vehs_ordered[0]-4), int(O_vehs_ordered[1]-4), int(O_vehs_ordered[2]-4), int(O_vehs_ordered[3]-4), int(O_vehs_ordered[4]-4), int(Distance_ordered[0]/6-1), int(Distance_ordered[1]/6-1), int(Distance_ordered[2]/6-1), int(Distance_ordered[3]/6-1), int(Distance_ordered[4]/6-1)]
        elif Activated_CAV_pair_num == 4:
            obj_opt = self.obj_opt_4CAV[int(self.Bandwidth_available-2), int(O_vehs_ordered[0]-4), int(O_vehs_ordered[1]-4), int(O_vehs_ordered[2]-4), int(O_vehs_ordered[3]-4), int(Distance_ordered[0]/6-1), int(Distance_ordered[1]/6-1), int(Distance_ordered[2]/6-1), int(Distance_ordered[3]/6-1)]
        elif Activated_CAV_pair_num == 3:
        ...
        if obj_opt == -1:           
            obj_modified = 0
            actions_modified = np.array([0,0,0,0,0,0])           
        ...
        return state_next, rew_n, done_n, obj_n, switch_n, reward_modified
