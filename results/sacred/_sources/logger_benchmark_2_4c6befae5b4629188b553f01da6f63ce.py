...
class RLLogger(object):
    def __init__(self, exp_name, _run, n_agents, n_adversaries, save_rate):
        ...
        self.episode_objs = [0.0]
        self.episode_switchs = [0.0]
    ...
    def record_episode_end(self, agents):
        ...
        self.episode_objs.append(0.0)
        self.episode_switchs.append(0.0)
    ...
    # def experiment_end(self,q_loss_all,pol_loss_all,train_steps_plot,episode_rewards_BF_optimal,episode_objs_BF_optimal,episode_switches_BF_optimal,episode_rewards_random,episode_objs_random,episode_switches_random,episode_rewards_modified):
    def experiment_end(self,q_loss_all,pol_loss_all,train_steps_plot,episode_rewards_modified):
        ...
        file_name = self.ex_path + '/' + current_time + '-learning_results.mat'
        print("file_name:", file_name)
        # sio.savemat(file_name, {"episode_rewards": np.array(self.episode_rewards),"episode_objs": np.array(self.episode_objs),\
        #     "episode_switchs": np.array(self.episode_switchs),"episode_rewards_BF_optimal": np.array(episode_rewards_BF_optimal),\
        #     "episode_objs_BF_optimal": np.array(episode_objs_BF_optimal),"episode_switches_BF_optimal": np.array(episode_switches_BF_optimal),\
        #     "episode_rewards_random": np.array(episode_rewards_random),"episode_objs_random": np.array(episode_objs_random),\
        #     "episode_switches_random": np.array(episode_switches_random),"q_loss_all": np.array(q_loss_all),"pol_loss_all": np.array(pol_loss_all),\
        #     "train_steps_plot": np.array(train_steps_plot),"episode_rewards_modified": np.array(episode_rewards_modified)})
        sio.savemat(file_name, {"episode_rewards": np.array(self.episode_rewards),"episode_objs": np.array(self.episode_objs),\
            "episode_switchs": np.array(self.episode_switchs),"q_loss_all": np.array(q_loss_all),"pol_loss_all": np.array(pol_loss_all),\
            "train_steps_plot": np.array(train_steps_plot),"episode_rewards_modified": np.array(episode_rewards_modified)})
    ...
    @property
    def cur_episode_obj(self):
        return self.episode_objs[-1]

    @cur_episode_obj.setter
    def cur_episode_obj(self, value):
        self.episode_objs[-1] = value

    @property
    def cur_episode_switch(self):
        return self.episode_switchs[-1]

    @cur_episode_switch.setter
    def cur_episode_switch(self, value):
        self.episode_switchs[-1] = value
