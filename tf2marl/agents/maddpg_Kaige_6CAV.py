...
class MADDPGAgent_Kaige_6CAV(AbstractAgent):
    def __init__(self, obs_space_n, act_space_n, agent_index, batch_size, buff_size, lr_c, lr_a, num_layer, num_units_c,  num_units_a, gamma,
        ...
        print("obs_space_n_before:", obs_space_n)
        # obs_shape_n = space_n_to_shape_n(obs_space_n)
        obs_shape_n = np.array(obs_space_n)
        print("obs_space_n_after:", obs_shape_n)
        print("act_space_n_before:", act_space_n)
        act_shape_n = space_n_to_shape_n(act_space_n)
        print("act_space_n_after:", act_shape_n)
        ...
        self.critic = MADDPGCriticNetwork(num_layer, num_units_c, lr_c, obs_shape_n, act_shape_n, act_type, agent_index)
        self.critic_target = MADDPGCriticNetwork(num_layer, num_units_c, lr_c, obs_shape_n, act_shape_n, act_type, agent_index)
        self.critic_target.model.set_weights(self.critic.model.get_weights())

        self.policy = MADDPGPolicyNetwork(num_layer, num_units_a, lr_a, obs_shape_n, act_shape_n[agent_index], act_type, 1,
                                          self.critic, agent_index)
        self.policy_target = MADDPGPolicyNetwork(num_layer, num_units_a, lr_a, obs_shape_n, act_shape_n[agent_index], act_type, 1,
                                                 self.critic, agent_index)
    ...
    def update_target_networks(self, tau):
        ...
        def update_target_network(net: tf.keras.Model, target_net: tf.keras.Model):
            ...
            for i in range(len(target_net_weights)):  # set tau% of target model to be new weights
                target_net_weights[i] = net_weights[i] * tau + target_net_weights[i] * (1 - tau)
            target_net.set_weights(target_net_weights)
    ...
    def update(self, agents, step):
        ...
        return [np.mean(td_loss), policy_loss]
...
class MADDPGPolicyNetwork(object):
    ...
    def get_action(self, obs):
        ...
        if self.use_gumbel:
            # print("self.use_gumbel:", self.use_gumbel)
    ...
    def train(self, obs_n, act_n):
        with tf.GradientTape() as tape:
            ...
            act_n = tf.unstack(act_n) # action of all agents from replay
            if self.use_gumbel:
                ...
                act_n[self.agent_index] = x # action of current agent is replaced using policy network
