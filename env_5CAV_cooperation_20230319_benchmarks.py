...
class Network():
    ...
    def reset(self):
        ...
        O_norm = (self.O_vehs_all - np.array([4,4,4,4,4]))/4
           
        
        ############################# Distance and channel gain ###############################################
        
        self.Distance_all = 6*np.ones(self.CAV_pair_num)
        Distance_norm = (self.Distance_all - np.array([6,6,6,6,6]))/24
    ...
    def step(self, curr_action):
        ...
        actions = np.array([np.argmax(actions_array[0]), np.argmax(actions_array[1]), np.argmax(actions_array[2]),np.argmax(actions_array[3]),np.argmax(actions_array[4])])
        ...
        if obj_opt == -1:           
            obj_modified = 0
            actions_modified = np.array([0,0,0,0,0])           
